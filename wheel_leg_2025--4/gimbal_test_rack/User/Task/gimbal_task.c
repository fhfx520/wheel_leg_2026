...
ADRC_Speed yaw_speed_adrc;
ADRC_Speed yaw_speed_adrc_define= {
    .Ts = 0.001f,          // 1ms 控制周期
    .b0 = 100.0f,          // 通过开环实验测得
    .wc = 12.0f,           // 期望闭环带宽 12 rad/s
    .wo = 60.0f,           // 观测器带宽 5倍 wc
    .u_max = 1.0f,         // 最大力矩指令 (根据电机规格)
};
...
gimbal_scale_t gimbal_scale = {
    ...
    .ecd_keyboard = 1,
    .angle_remote = 0.000007f,//测试架1111修改变大	4号上是0.000007f
...
static void gimbal_init(void)
{
	...
	//1111测试架pitch6020初始化
//	pid_init(&gimbal.pit_angle.pid, CHANG_I_RATE, 24.0f, 0.25f, 0, 25, 50);
//	pid_init(&gimbal.pit_spd.pid, CHANG_I_RATE, 10000.0f, 0.0f, 0, 8000.0f, 25000.0f);

	pid_init(&gimbal.pit_angle.pid, CHANG_I_RATE, 22.0f, 0.25f, 0, 25, 50);
	pid_init(&gimbal.pit_spd.pid, CHANG_I_RATE, 8000.0f, 0.0f, 0, 8000.0f, 25000.0f);
...
//	ADRC_Speed_Init(&yaw_speed_adrc, 0.001f,1.0f, 1.0f, 1.0f, 0.2f);
}
float yaw_err;
...
static void gimbal_pid_calc(void)
{
    ...
    //速度环反馈 陀螺仪
    //此yaw_err用于云台pit限幅
    yaw_err = circle_error(CHASSIS_YAW_OFFSET, yaw_motor.position, 2 * PI);
...
//    gimbal.yaw_angle.ref = FGT_agl_calc(&yaw_test);
	...
	gimbal.yaw_vision_vel.fdb = gimbal_imu.wz;//同济视觉双外环测试
...
//   gimbal.yaw_spd.ref = pid_calc(&gimbal.yaw_angle.pid, gimbal.yaw_angle.fdb + yaw_err, gimbal.yaw_angle.fdb) + pid_calc(&gimbal.yaw_vision_vel.pid,gimbal.yaw_vision_vel.ref,gimbal.yaw_vision_vel.fdb); //同济视觉测试双外环pid控制   
//    gimbal.yaw_spd.ref = 0;  
	
	   gimbal.yaw_spd.ref = pid_calc(&gimbal.yaw_angle.pid, gimbal.yaw_angle.fdb + yaw_err, gimbal.yaw_angle.fdb) + kkkkk * vision.rx[0].data.yaw_vel; //同济视觉测试双外环pid控制   
...
//	  gimbal.yaw_output = -ADRC_Speed_Update(&yaw_speed_adrc, gimbal.yaw_spd.ref, gimbal_imu.wz); //LADRC
...
//		dji_motor_set_torque(&yaw_motor, 0);   	
		gimbal.yaw_output = 0.0f;		
		dji_motor_set_torque(&pit_motor, 0); //1111测试架pitch为6020		
//		dm_motor_set_control_para(&pit_motor,0,0,0,0,0);
//		dm_motor_set_control_para(&yaw_motor,0,0,0,0,0);
    }       
    else{
		dji_motor_set_torque(&yaw_motor, 1.0f*gimbal.yaw_output);
		dji_motor_set_torque(&pit_motor, 1.0f*gimbal.pit_output);     
//		dm_motor_set_control_para(&yaw_motor,0,0,0,0,1.0f*gimbal.yaw_output);//极性待定
//		dm_motor_set_control_para(&yaw_motor,0,0,0,0,1.0f*gimbal.yaw_output);//测试架1111

//		dm_motor_set_control_para(&pit_motor,0,0,0,0,1.0f*gimbal.pit_output);
    }
}
float diff_up_limit = 0.5f;
float diff_down_limit = 0.05f;
        ...
        case AIMING: {//识别到目标
            if (vision.new_frame_flag) {
               ...
               gimbal.yaw_angle.ref = vision.target_yaw_angle;
				...
				//同济代码测试
//					
					// gimbal.pit_angle.ref = vision_rx.target_pit_angle;
					// gimbal.yaw_angle.ref = vision_rx.target_yaw_angle;	
//					gimbal.yaw_vision_vel.ref = vision_rx.target_yaw_vel;
				//					gimbal.pit_angle.pid.out_max = ABS(vision_rx.target_pit_vel); //wy不听测测看
//					if(fabsf(gimbal.yaw_angle.ref - gimbal.yaw_angle.fdb) > diff_up_limit)
//					{
//						gimbal.yaw_angle.pid.out_max = 100;
//						gimbal.yaw_spd.pid.kp = 0.3f;
//					}
//					else if(fabsf(gimbal.yaw_angle.ref - gimbal.yaw_angle.fdb) > diff_down_limit)
//					{
//						gimbal.yaw_angle.pid.out_max = (100 - vision_rx.target_yaw_vel) / (diff_up_limit - diff_down_limit) * fabsf(gimbal.yaw_angle.ref - gimbal.yaw_angle.fdb);
//						gimbal.yaw_spd.pid.kp = (0.3f - 0.5f) / (diff_up_limit - diff_down_limit) * fabsf(gimbal.yaw_angle.ref - gimbal.yaw_angle.fdb);
//					}
//					else
//					{
//						gimbal.yaw_angle.pid.out_max = ABS(vision_rx.target_yaw_vel);
//						gimbal.yaw_spd.pid.kp = 0.5f;
//					}
//					
//					if(gimbal.pit_angle.pid.out_max > 50)
//						gimbal.pit_angle.pid.out_max = 50;
//					if(gimbal.yaw_angle.pid.out_max > 100)
//						gimbal.yaw_angle.pid.out_max = 100;
            }
            break;
        }	
    ...
    for(;;) {
        ...
        switch (ctrl_mode) {
            case PROTECT_MODE: {	
				...
				yaw_err_up = circle_error(CHASSIS_YAW_OFFSET, yaw_motor.position , 2 * PI);
				
//				if(!gimbal.start_up)
//				{
//					if(fabsf(yaw_err_up) < PI / 2.0f)
//						gimbal.yaw_angle.ref = gimbal_imu.yaw + yaw_motor.position - CHASSIS_YAW_OFFSET;//yaw轴反馈值+电机与前方灯条差值
//					else
//						gimbal.yaw_angle.ref = gimbal_imu.yaw + yaw_motor.position - CHASSIS_YAW_OFFSET - PI;	
//				}
				gimbal.yaw_angle.ref = gimbal_imu.yaw;//1111测试架不需要startup
            ...
            case REMOTER_MODE: {
                if (( rc_fsm_check(RC_LEFT_LD) && rc_fsm_check(RC_RIGHT_RD) )|| 1){ //遥控器开启视觉
