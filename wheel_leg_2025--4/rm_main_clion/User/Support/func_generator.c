 ...
 * @retval    void
 */
void FGT_sin_init(FGT_sin_t* sin, float Td, float dc, float T, float A, float phi,float max,float min)
{
    sin->Td = Td;
    sin->time = 0;
    sin->dc = dc;
...
float FGT_sin_calc(FGT_sin_t* sin)
{
    ...
    if(try_try % 1 == 0){
     if(sin->time > 500 && sin->time < 2500)  
     LIMIT_MIN_MAX(sin->out, sin->min, sin->max/2); 
    else        
    LIMIT_MIN_MAX(sin->out, sin->min, sin->max);    
    }
    else
    LIMIT_MIN_MAX(sin->out, 0, 0);    
