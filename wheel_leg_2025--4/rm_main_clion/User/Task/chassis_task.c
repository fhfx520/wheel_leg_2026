...
//    key_scan(KEY_CHASSIS_PRONE);
    ...
    switch (ctrl_mode) {
    case PROTECT_MODE: { //能量模式和保护模式下，底盘�?�为相同
    ...
    case REMOTER_MODE: {
        ...
        /* 底盘小陀螺模�? */
        static uint8_t spin_flag; //单�?�触发使能标�?
        ...
        /* 遥控器注释底�? */
        if (rc_fsm_check(RC_LEFT_RU) || rc_fsm_check(RC_RIGHT_RU)) { //遥控器注释底�?
            chassis.mode = CHASSIS_MODE_PROTECT;
        }
        break;
    }
    case KEYBOARD_MODE: { //�?盘模式下�?(跟随，陀螺，迎敌三�?�模式相互切�?),(跟随与补给模式相互切�?)  KB_R
        /* 底盘模式切换 */
        if (last_ctrl_mode == PROTECT_MODE )
            chassis.recover_flag = 1;			
        switch (chassis.mode) {
        case CHASSIS_MODE_KEYBOARD_FOLLOW: { //�?盘跟随模式下
...
static void chassis_data_input(void)
{
    ...
    switch (chassis.mode) {
        ...
        case CHASSIS_MODE_REMOTER_FOLLOW:  //底盘遥控跟随模式 底盘遥控陀螺模�?
        ...
        case CHASSIS_MODE_REMOTER_ROTATE2: {
            ...
            chassis.input.vx = -rc.ch4 * chassis_scale.remote; //�?标速度
            chassis.input.vy = rc.ch3 * chassis_scale.remote;
						wlr.ctrl_mode = 2; //�?腿模�?
            
           //高度模式
           if (wlr.ctrl_mode == 2) {  //�?腿模式下才可控制腿长
			   if (rc.sw2 == RC_UP) {
                   wlr.jump_flag = 0;
                   wlr.high_flag = 0;
                   wlr.power_flag = 1;
...
//											 wlr.sky_flag = 1; 
        ...
        case CHASSIS_MODE_KEYBOARD_FOLLOW:  //跟随模式�?以跳�?  底盘�?盘跟�?/陀�?/迎敌模式
						wlr.ctrl_mode = 2; //�?腿模�?
...
//            }
                ...
                chassis_scale.keyboard = 1.0f;  //迎敌模式			
								 ...
								 if(KEY_PRESS_POWER && !wlr.jump_flag) {
										...
										chassis_scale.keyboard = 1.5f;  //通模式下----------------------2025.3.4 改2.5
								}
...
float fsjl;
float up_ready;
static void chassis_self_rescue(void)//翻车�?�?
{
...
static void chassis_data_output(void)
{
    ...
    if (wlr.ctrl_mode == 0) {//保护模式
        ...
        if (wlr.prone_flag) {
            ...
            if(chassis.recover_flag == 1)//恢�?��??一象限
                chassis_self_rescue();
