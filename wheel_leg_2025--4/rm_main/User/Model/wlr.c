...
//				wlr.high_set =	ramp_calc(&sky_ramp,0.5f);
				wlr.high_set = 0.40f;
				x3_balance_zero = 0.2;
				if (fabs(0.38f - vmc[0].L_fdb) < 0.02f && fabs(0.38f - vmc[1].L_fdb) < 0.02f)
					wlr.sky_cnt ++;
				if (wlr.sky_cnt > 5){
					wlr.sky_cnt = 0;
					wlr.sky_flag = 3;
...
/******************************状态选择********************************/
	...
	for (int i = 0; i < 2; i++) {
        ...
        if (wlr.side[0].fly_flag && wlr.side[1].fly_flag && wlr.jump_flag==0 && !chassis.recover_flag && wlr.sky_over == 0) {
			...
			Fy_temp = pid_calc(&pid_leg_recover[i], wlr.recover_length, vmc[i].L_fdb); 	
...
//		 vmc_inverse_solution(&vmc[i], wlr.high_set, PI / 2 + x3_balance_zero, wlr.side[i].T0, wlr.side[i].Fy);
    ...
    for (int i = 0; i < 2; i++) {
		...
		if ( wlr.crash_flag || wlr.jump_flag == 2 || wlr.jump_flag == 3)				//两条腿撞上台阶 或 完成上台阶 或 完成上台阶
			...
			lqr.U_ref[i] *= 0.8f;
		else if(chassis.recover_flag >= 1 || wlr.sky_flag == 4 || wlr.sky_over )	// 或 跳跃在空中收腿 或 落地伸腿缓冲
			lqr.U_ref[i] *= 0.0f;		
