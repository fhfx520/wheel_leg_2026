...
volatile float x3_balance_zero = 0.0f;//腿摆角角度偏置   负值：腿摆角向膝关节方向偏	正值：腿摆角向膝关节反方向偏 
volatile float x5_balance_zero = 0.0f;//机身倾角偏置     负值：抬头				正值：低头
const float x3_balance_zero_normal = 0.0f; //车头朝前正常情况偏置
const float sky_yaw_offset = -0.1f;//跳跃yaw偏置 不知为何导致车起跳的时候会往右扭，这里给个向左的偏置去抵消向右扭的趋势
...
//	power_limit_rotate = power_control_target_Vrotate();
	power_limit_velocity = 2.5f;
	power_limit_rotate = 13.0f;
    ...
    if(wlr.sky_flag == WLR_SKY_FOLDING)
		...
		data_limit(&lqr.X_ref[1],-2.5f,2.5f);
...
//    } 
	...
	else if (wlr.high_flag == 1 || wlr.stair_flag == WLR_STAIR_LANDING){//中腿长
        wlr.side[index].Fy = pid_calc(&pid_L_test[index], tlm.l_ref[index], vmc[index].L_fdb) - 30.0f
                              + WLR_SIGN(index) * (wlr.roll_offs + wlr.inertial_offs);
    }
	else {//低腿长
...
void wlr_control(void)
{
	...
	if(wlr.sky_flag == WLR_SKY_EXTENDING && !wlr.double_flag)
