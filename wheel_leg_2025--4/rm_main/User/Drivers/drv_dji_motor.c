    ...
    for (node = object_list.next; node != &(object_list); node = node->next) {
        ...
        if (object->motor_type == DJI_6020_MOTOR) {
            if (object->can_id < 0x209){
                motor_msg[object->can_channel][2].id = 0xADC;//0x1FE
                ...
                motor_msg[object->can_channel][3].id = 0xADC;//0x2FE
...
{
    dji_motor_fill_data();
    for (can_channel_e can_channel = CAN_CHANNEL_1; can_channel != (CAN_CHANNEL_NUM-1); can_channel++) {//排除can3电机
        for (int i = 0; i < 4; i++) {
