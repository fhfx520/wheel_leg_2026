...
static void chassis_init()
{
    ...
    wlr_init();
	//joint_motor_reset();//dm 暂不需要，后续应该是重新使能
...
//    key_scan(KEY_CHASSIS_UNFOLLOW);
    ...
    case KEYBOARD_MODE: { //键盘模式下：(跟随，陀螺，迎敌三种模式相互切换),(跟随与补给模式相互切换)
        ...
        case CHASSIS_MODE_KEYBOARD_FOLLOW: { //键盘跟随模式下
            ...
            } else if (kb_status[KEY_CHASSIS_UNFOLLOW] && 0) { //进入不跟随模式
        ...
        case CHASSIS_MODE_KEYBOARD_UNFOLLOW: { //键盘不跟随模式下
            if (!kb_status[KEY_CHASSIS_UNFOLLOW] || 1) { //恢复跟随模式
                chassis.mode = CHASSIS_MODE_KEYBOARD_FOLLOW;
...
//            }
        ...
        case CHASSIS_MODE_KEYBOARD_ROTATE: {
            ...
            if (chassis.mode == CHASSIS_MODE_REMOTER_ROTATE1) {
                wlr.yaw_ref += (float)(CHASSIS_ROTATE_SPEED);
...
//    wlr.yaw_err = circle_error(wlr.yaw_ref, wlr.yaw_fdb, 2 * PI);
    if (chassis.mode == CHASSIS_MODE_REMOTER_ROTATE1 ||
        chassis.mode == CHASSIS_MODE_REMOTER_ROTATE2)
        wlr.yaw_ref = wlr.yaw_fdb + 5.0f;
   wlr.yaw_ref = wlr.yaw_fdb + 1.0f * wlr.yaw_err;
//    wlr.yaw_ref = wlr.yaw_fdb + (rc.ch1/660)*(PI/1.5f);
...
//    wlr.wy_fdb      = -chassis_imu.wy;//
    ...
    //电机数据输入
    wlr.side[0].q2 =  joint_motor[1].position - joint_motor[1].zero_point;//电机原始数据
    ...
    wlr.side[1].q2 =  -(joint_motor[3].position - joint_motor[3].zero_point);//电机原始数据
...
static void chassis_self_rescue(void)//翻车自救
{
    ...
    if ((vmc[0].quadrant == 3 || vmc[0].quadrant == 2) && chassis.rescue_cnt_L < 50) {
        dm_motor_set_control_para(&joint_motor[0], 0, 3, 0, 5, 0);//0.03 0.5
        dm_motor_set_control_para(&joint_motor[1], 0, 0, 0, 0, 0);	
        chassis.rescue_cnt_L = 0;
    }
    else
        chassis.rescue_cnt_L++;       
    if ((vmc[1].quadrant == 3 || vmc[1].quadrant == 2) && chassis.rescue_cnt_R < 50) {
        dm_motor_set_control_para(&joint_motor[2], 0, -3, 0, 5, 0);//0.03 0.5
...
//    if (chassis_imu.rol < -0.2)
//    {
//        dm_motor_set_control_para(&joint_motor[0], 0, 4, 0, 5, 0);//0.03 0.5
//        dm_motor_set_control_para(&joint_motor[1], 0, 0, 0, 0, 0);

//    }else if (chassis_imu.rol > 0.2)
//    {
//        dm_motor_set_control_para(&joint_motor[2], 0, -4, 0, 5, 0);//0.03 0.5
//        dm_motor_set_control_para(&joint_motor[3], 0, 0, 0, 0, 0);
//    }
    if(chassis.rescue_cnt_L > 300 && chassis.rescue_cnt_R > 300) {
        dm_motor_set_control_para(&joint_motor[0], 0, 0, 0, 0, 0.3f*wlr.side[0].T1);//0.03 0.5
        dm_motor_set_control_para(&joint_motor[1], 0, 0, 0, 0, wlr.side[0].T2); 
        dm_motor_set_control_para(&joint_motor[2], 0, 0, 0, 0,-0.3f*wlr.side[1].T1);
        dm_motor_set_control_para(&joint_motor[3], 0, 0, 0, 0,-wlr.side[1].T2); 
    }
    
    
    if (chassis.rescue_cnt_L > 350 && chassis.rescue_cnt_R > 350 && fabs(vmc[0].L_fdb - wlr.recover_length) < 0.05f && fabs(vmc[1].L_fdb - wlr.recover_length) < 0.05f) {
...
static void chassis_data_output(void)
{
    if (wlr.ctrl_mode == 0) {//保护模式
        ...
        if (wlr.prone_flag) {		
            ...
            if(chassis.recover_flag == 1)//恢复第一象限
                chassis_self_rescue();
