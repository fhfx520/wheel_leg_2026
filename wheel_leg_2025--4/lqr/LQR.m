...
% 腿长：0.18m
% Q = diag([3000, 5000 , 50000, 4000,    120000 , 500, 120000 , 500,    150000, 1000]);
% R = diag([400, 400, 100, 100]);

% 腿长0.21
Q = diag([1000, 3000, 20000, 1000, 40000, 500, 40000, 500, 120000, 1000]);
R = diag([400, 400, 100, 100]);%0.22 22调这个QR

% 腿长0.13
% Q = diag([1000, 4000, 15000, 1000, 40000, 100, 40000, 100, 80000, 500]);
% R = diag([300, 300, 100, 100]);%0.13 调这个QR
