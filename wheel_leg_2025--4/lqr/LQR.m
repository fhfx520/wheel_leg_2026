...
% 腿长：0.23m 2.6
% Q = diag([1000, 2000 , 40000, 4000, 300000 , 500, 300000 , 500, 200000, 1000]);
% R = diag([250, 250, 100, 100]);

% 腿长：0.13
% Q = diag([1000, 20000, 50000, 1000, 30000, 100, 30000, 100, 80000, 500]);
% R = diag([400, 400, 100, 100]);%0.13 调这个QR
% L_ref = 0.13;

% 腿长：0.16
Q = diag([3000, 6000 , 60000, 4000,    120000 , 500, 120000 , 500,    80000, 1000]);
R = diag([1000, 1000, 100, 100]);
L_ref = 0.16;

% 腿长：0.21
% Q = diag([700, 15000, 50000, 1000, 300000, 1000, 300000, 1000, 200000, 1000]);
% R = diag([400, 400, 50, 50]);%0.22 22调这个QR
% L_ref = 0.21;

% 腿长：0.30
% Q = diag([500, 5000, 30000, 1000, 80000, 1000, 80000, 1000, 80000, 1000]);
% R = diag([400, 400, 80, 80]);
% L_ref = 0.30;

% 跳跃
% Q = diag([3000, 6000 , 50000, 4000,    120000 , 500, 120000 , 500,    150000, 1000]);
% R = diag([600, 600, 100, 100]);

% 小陀螺
% Q = diag([3000, 6000 , 3000, 1000,    80000 , 500, 80000 , 500,    150000, 1000]);
% R = diag([2000, 2000, 100, 100]);
% L_ref = 0.15;



[L.L, L.L_l, L.L_r] = deal(L_ref);            %腿长
