...
/* USER CODE BEGIN 0 */
uint32_t HRTIM_DMA_Buffer[2];			//占空比DMA发送数组
/* USER CODE END 0 */
...
void MX_HRTIM1_Init(void)
{
  ...
  if (HAL_HRTIM_ADCPostScalerConfig(&hhrtim1, HRTIM_ADCTRIGGER_3, 0x00) != HAL_OK)
  {
    Error_Handler();
  }
  if (HAL_HRTIM_ADCPostScalerConfig(&hhrtim1, HRTIM_ADCTRIGGER_3, 0x00) != HAL_OK)
  ...
  pTimerCfg.DMARequests = HRTIM_TIM_DMA_CMP3;
  pTimerCfg.DMASrcAddress = (uint32_t)&HRTIM_DMA_Buffer[0];
  pTimerCfg.DMADstAddress = (uint32_t)&(hhrtim1.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_A].CMP1xR);
  ...
  pTimerCfg.DMASrcAddress = (uint32_t)&HRTIM_DMA_Buffer[1];
  pTimerCfg.DMADstAddress = (uint32_t)&(hhrtim1.Instance->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_B].CMP1xR);
  ...
  pCompareCfg.CompareValue = 6800-1000;
...
void HAL_HRTIM_MspInit(HRTIM_HandleTypeDef* hrtimHandle)
{
  ...
  /* USER CODE END HRTIM1_MspInit 0 */
    ...
    /* HRTIM1 DMA Init */
    /* HRTIM1_A Init */
    hdma_hrtim1_a.Instance = DMA1_Channel7;
    ...
    /* HRTIM1_B Init */
    hdma_hrtim1_b.Instance = DMA1_Channel8;
